    ...
    const AdminLayout = ({ children }) => {
      ...
      return (
        <div className="flex min-h-screen transition-colors duration-300 bg-gray-100 dark:bg-gray-900">
          ...
          >
            <Link to="/admin/dashboard" className="flex items-center justify-center mb-8 text-2xl font-bold text-center text-transparent gradient-highlands bg-clip-text">
              ...
              <div className="flex items-center justify-between mb-2">
                {currentUser && (
                  <div className="flex items-center space-x-2 text-sm">
                     <img  src={`https://api.dicebear.com/7.x/initials/svg?seed=${currentUser.name || currentUser.email}`} alt="admin avatar" className="border-2 rounded-full h-7 w-7 border-primary" />
                    <span>{currentUser.name}</span>
                  </div>
                )}
                <Button onClick={toggleTheme} variant="ghost" size="icon" className="text-gray-400 h-7 w-7 hover:text-white">
                  {theme === 'light' ? <Moon className="w-4 h-4" /> : <Sun className="w-4 h-4" />}
                </Button>
              </div>
              <Button onClick={handleLogout} variant="ghost" className="justify-start w-full text-red-400 hover:bg-red-500/20 hover:text-red-300">
