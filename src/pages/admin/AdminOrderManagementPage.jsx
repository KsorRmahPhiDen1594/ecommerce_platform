...
const mockAdminOrders = Array(10)
  .fill(null)
  .map((_, i) => {
...
const AdminOrderManagementPage = () => {
  ...
  const handleUpdateStatus = () => {
    ...
    toast({
      title: 'Thành công',
      description: `Đã cập nhật trạng thái đơn hàng ${selectedOrder.id} thành ${newStatus}.`,
    });
      ...
      <Card className='shadow-lg'>
        ...
        <CardContent className='p-0'>
                        ...
                        order.status === 'Đã giao'
                          ...
                          : order.status === 'Đang giao'
                            ...
                            : order.status === 'Đang xử lý'
                              ? 'secondary'
                              : order.status === 'Đã hủy' || order.status === 'Hoàn tiền'
                                ? 'destructive'
                                : 'secondary'
                      }
                      className={
                  ...
                  <TableCell className='text-right'>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align='end'>
                        ...
                        <DropdownMenuItem onClick={() => openEditStatusModal(order)}>
          ...
          {filteredOrders.length === 0 && (
            <p className='p-4 text-center text-muted-foreground'>Không tìm thấy đơn hàng nào.</p>
          )}
        </CardContent>
        <CardFooter className='p-4 border-t'>
          <div className='text-xs text-muted-foreground'>
            Hiển thị <strong>{filteredOrders.length}</strong> trên tổng số{' '}
          ...
          <DialogContent className='sm:max-w-lg md:max-w-2xl'>
            ...
            <div className='py-4 max-h-[60vh] overflow-y-auto pr-2 space-y-4'>
              <div className='grid grid-cols-2 text-sm gap-x-4 gap-y-2'>
                <p>
                  <strong className='text-muted-foreground'>Khách hàng:</strong>{' '}
                  {selectedOrder.customerName}
                </p>
                <p>
                  <strong className='text-muted-foreground'>Ngày đặt:</strong> {selectedOrder.date}
                </p>
                <p>
                  <strong className='text-muted-foreground'>Tổng tiền:</strong>{' '}
                  <span className='font-bold text-primary'>
                    {parseInt(selectedOrder.totalAmount).toLocaleString()}₫
                  </span>
                </p>
                <p>
                  <strong className='text-muted-foreground'>Thanh toán:</strong>{' '}
                  {selectedOrder.paymentMethod}
                </p>
                <p className='col-span-2'>
                  <strong className='text-muted-foreground'>Địa chỉ giao:</strong>{' '}
                  {selectedOrder.shippingAddress}
                </p>
                <p className='col-span-2'>
                  <strong className='text-muted-foreground'>Trạng thái:</strong>{' '}
              ...
              <div>
                ...
                <ul className='space-y-2'>
                  {selectedOrder.items.map((item, index) => (
                    <li
                      key={index}
                      className='flex items-center justify-between p-2 text-sm rounded-md bg-muted/50 dark:bg-slate-800/50'
                    >
                      <div>
      ...
      {selectedOrder && (
        <Dialog open={isEditStatusModalOpen} onOpenChange={setIsEditStatusModalOpen}>
          <DialogContent className='sm:max-w-md'>
            ...
            <div className='py-4'>
              <Label htmlFor='order-status' className='block mb-2'>
                Trạng thái mới
              </Label>
