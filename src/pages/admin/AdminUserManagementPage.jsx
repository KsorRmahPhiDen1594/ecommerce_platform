...
const AdminUserManagementPage = () => {
  ...
  const handleEditUser = () => {
    ...
    const updatedUsers = users.map((u) =>
      u.id === currentUserToEdit.id
        ? { ...u, ...newUserData, password: newUserData.password || u.password }
        : u,
    );
    setUsers(updatedUsers);
    localStorage.setItem('users', JSON.stringify(updatedUsers));
  ...
  const toggleUserStatus = (userId) => {
    ...
    toast({
      ...
      description: `Người dùng ${user.name} đã được ${user.status === 'active' ? 'mở khóa' : 'khóa'}.`,
    });
  };

  const deleteUser = (userId) => {
    if (
      window.confirm('Bạn có chắc chắn muốn xóa người dùng này? Hành động này không thể hoàn tác.')
    ) {
      ...
      <Card className='shadow-lg'>
        ...
        <CardContent className='p-0'>
            ...
            <TableBody>
              {filteredUsers.map((user) => (
                <TableRow key={user.id} className='hover:bg-muted/50'>
                  <TableCell>
                    <img
                      src={`https://api.dicebear.com/7.x/initials/svg?seed=${user.name}&backgroundColor=${user.role === 'admin' || user.role === 'superadmin' ? 'ff7f50' : '60a5fa'}&radius=50`}
                      alt='avatar'
          ...
          {filteredUsers.length === 0 && (
            <p className='p-4 text-center text-muted-foreground'>Không tìm thấy người dùng nào.</p>
          )}
        </CardContent>
      ...
      <Dialog open={isAddUserModalOpen} onOpenChange={setIsAddUserModalOpen}>
        <DialogContent className='sm:max-w-[425px]'>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAddUser} className='gradient-highlands text-primary-foreground'>
              Thêm người dùng
            </Button>
          </DialogFooter>
        </DialogContent>
