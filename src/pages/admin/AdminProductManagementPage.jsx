...
const mockAdminProducts = Array(15)
  .fill(null)
  .map((_, i) => ({
    id: `prod${i + 1}`,
    name: `Sản phẩm mẫu ${i + 1} cho Admin`,
    category: ['Điện thoại', 'Laptop', 'Thời trang', 'Đồ gia dụng', 'Sách'][
...
const AdminProductManagementPage = () => {
  ...
  const toggleProductStatus = (productId) => {
    ...
    toast({
      ...
      description: `Sản phẩm ${product.name} đã được ${product.status === 'active' ? 'ẩn' : 'hiển thị'}.`,
    });
  };
      ...
      <Card className='shadow-lg'>
        ...
        <CardContent className='p-0'>
                ...
                <TableRow key={product.id} className='hover:bg-muted/50'>
                  <TableCell>
                    <img
                      ...
                      alt={product.name}
                      src='https://images.unsplash.com/photo-1588099768531-a72d4a198538?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=200&q=80'
                    />
                  </TableCell>
                  ...
                  <TableCell>
                    <Badge
                      ...
                      className={
                        product.status === 'active' ? 'border-green-500 text-green-600' : ''
                      }
                    >
          ...
          {filteredProducts.length === 0 && (
            <p className='p-4 text-center text-muted-foreground'>Không tìm thấy sản phẩm nào.</p>
          )}
        </CardContent>
        <CardFooter className='p-4 border-t'>
          <div className='text-xs text-muted-foreground'>
            Hiển thị <strong>{filteredProducts.length}</strong> trên tổng số{' '}
      ...
      <Dialog open={isAddEditModalOpen} onOpenChange={setIsAddEditModalOpen}>
        <DialogContent className='sm:max-w-lg'>
          <DialogHeader>
            ...
            <DialogDescription>
              {currentProduct
                ? `Cập nhật thông tin cho sản phẩm: ${currentProduct.name}`
                : 'Điền thông tin chi tiết cho sản phẩm mới.'}
            </DialogDescription>
          </DialogHeader>
          <div className='grid gap-4 py-4 max-h-[60vh] overflow-y-auto pr-2'>
            ...
            <div className='grid items-center grid-cols-4 gap-4'>
              <Label htmlFor='prod-category' className='text-right'>
                Danh mục
              </Label>
              <Select
            ...
            <div className='grid items-center grid-cols-4 gap-4'>
              <Label htmlFor='prod-stock' className='text-right'>
                Tồn kho
              </Label>
              <Input
                id='prod-stock'
            ...
            <div className='grid items-start grid-cols-4 gap-4'>
              <Label htmlFor='prod-description' className='pt-2 text-right'>
                Mô tả
              </Label>
              <Textarea
                id='prod-description'
            ...
            <div className='grid items-center grid-cols-4 gap-4'>
              <Label htmlFor='prod-image' className='text-right'>
                Ảnh (placeholder)
              </Label>
              <Input
                id='prod-image'
                name='image_placeholder'
                value={productFormData.image_placeholder}
                onChange={handleInputChange}
                className='col-span-3'
                placeholder='Mô tả ảnh hoặc URL'
              />
            </div>
          </div>
          <DialogFooter>
