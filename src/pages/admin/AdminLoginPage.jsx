  ...
  const handleSubmit = async (e) => {
    ...
    if (!email || !password) {
      toast({
        variant: 'destructive',
        title: 'Lỗi',
        description: 'Vui lòng nhập email và mật khẩu Admin.',
      });
      return;
    }
      ...
      >
        <Card className='shadow-2xl bg-slate-800/70 border-primary/30 backdrop-blur-md text-slate-100'>
          <CardHeader className='p-6 text-center'>
            <Link
              to='/'
              className='flex items-center justify-center mb-4 text-3xl font-bold text-transparent gradient-highlands bg-clip-text'
            >
          ...
          <CardContent className='px-6 pb-6 space-y-5'>
            <form onSubmit={handleSubmit} className='space-y-4'>
              ...
              <div className='space-y-1.5'>
                <Label htmlFor='admin-otp' className='text-slate-300'>
                  Mã OTP (Nếu có)
                </Label>
                <div className='relative'>
              ...
              <Button
                type='submit'
                className='w-full py-3 gradient-highlands text-primary-foreground text-md'
                disabled={loading}
              >
                {loading ? (
                  'Đang xác thực...'
