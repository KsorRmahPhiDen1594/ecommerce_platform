          ...
          <Card className="shadow-md sticky top-24">
            <CardHeader className="border-b p-4">
              <CardTitle className="flex items-center text-lg">
                <Filter className="mr-2 h-5 w-5 text-primary" /> Bộ lọc tìm kiếm
              </CardTitle>
            </CardHeader>
            <CardContent className="p-4 max-h-[calc(100vh-10rem)] overflow-y-auto">
              ...
              <FilterSection title="Đánh giá">
                {[5, 4, 3].map(star => (
                  <div key={star} className="flex items-center space-x-2">
                    ...
                    />
                    <Label htmlFor={`star-${star}`} className="ml-1 flex items-center text-sm">
                      {Array(star)
                        .fill(0)
                        .map((_, i) => (
              ...
              {['Phổ biến', 'Bán chạy', 'Giá: Tăng dần', 'Giá: Giảm dần'].map(opt => (
                <Button
                  ...
                  className={`text-xs md:text-sm px-2 py-1 md:px-3 md:py-1.5 ${
                    sortOption === opt
                      ? 'gradient-shopee text-primary-foreground'
