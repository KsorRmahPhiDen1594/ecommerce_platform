...
const ProductDetailPage = () => {
  ...
  if (error || !product) return <div className="text-center py-8">{error || 'Sản phẩm không tồn tại.'}</div>;
  ...
  return (
    <div className="container mx-auto p-4 space-y-6">
            ...
            <div className="relative">
              {product.images[mainMediaIndex].type === 'image' ? (
                ...
                <div className="relative">
                  ...
                  <PlayCircle className="absolute inset-0 m-auto h-20 w-20 text-white cursor-pointer" onClick={() => alert('Play video')} />
                </div>
              )}
              {/* Nav arrows */}
              <Button size="icon" variant="ghost" className="absolute top-1/2 left-0 -translate-y-1/2" onClick={() => setMainMediaIndex((mainMediaIndex-1+product.images.length)%product.images.length)}><ChevronLeft /></Button>
              <Button size="icon" variant="ghost" className="absolute top-1/2 right-0 -translate-y-1/2" onClick={() => setMainMediaIndex((mainMediaIndex+1)%product.images.length)}><ChevronRight /></Button>
            </div>
            <div className="grid grid-cols-5 gap-2">
                      ...
                      <CardContent className="flex gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-yellow-500">{r.rating} <Star className="inline h-4 w-4" /></span>
                          </div>
                          <p>{r.comment}</p>
                        </div>
                      </CardContent>
      ...
      {/* Related Products */}
      <div>
        <h2 className="text-xl font-bold mb-3">Sản phẩm liên quan</h2>
