...
const ProductDetailPage = () => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        const mappedProduct = {
          ...
          returnPolicy:
            'Đổi trả trong 7 ngày nếu có lỗi từ nhà sản xuất hoặc không vừa ý (có điều kiện).',
  ...
  if (error || !product)
    return <div className='py-8 text-center'>{error || 'Sản phẩm không tồn tại.'}</div>;
          ...
          <div className='space-y-2 lg:col-span-2'>
            <div className='relative'>
              {product.images[mainMediaIndex].type === 'image' ? (
                <img
                  src={product.images[mainMediaIndex].src}
                  alt={product.name}
                  className='w-full cursor-pointer'
                  onClick={() => openImageModal(product.images[mainMediaIndex].src)}
                />
              ) : (
                <div className='relative'>
                  <img
                    src={product.images[mainMediaIndex].thumbnail}
                    alt={product.name}
                    className='w-full opacity-50'
                  />
                  <PlayCircle
                    className='absolute inset-0 w-20 h-20 m-auto text-white cursor-pointer'
              ...
              <Button
                ...
                onClick={() =>
                  setMainMediaIndex(
                    (mainMediaIndex - 1 + product.images.length) % product.images.length,
                  )
                }
              >
                <ChevronLeft />
              ...
              <Button
                ...
                onClick={() => setMainMediaIndex((mainMediaIndex + 1) % product.images.length)}
              >
                <ChevronRight />
              </Button>
            </div>
            <div className='grid grid-cols-5 gap-2'>
              {product.images.map((img, idx) => (
                <img
                  key={idx}
                  src={img.type === 'image' ? img.src : img.thumbnail}
                  alt={img.alt}
                  className={`cursor-pointer border-2 ${mainMediaIndex === idx ? 'border-primary' : 'border-transparent'}`}
            ...
            <Tabs defaultValue='description' className='mt-6'>
                        ...
                        <div>
                          <div className='flex items-center gap-2'>
                            ...
                            <span className='text-yellow-500'>
                              {r.rating} <Star className='inline w-4 h-4' />
                            </span>
                          </div>
                          <p>{r.comment}</p>
                        </div>
              ...
              <TabsContent value='shipping'>
                <div className='space-y-2'>
                  <p>
                    <Truck className='inline mr-1' /> {product.shippingInfo}
                  </p>
                  <p>
                    <ShieldCheck className='inline mr-1' /> {product.returnPolicy}
                  </p>
                </div>
              </TabsContent>
            </Tabs>
