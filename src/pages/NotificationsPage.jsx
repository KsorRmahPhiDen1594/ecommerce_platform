...
const initialNotifications = {
  orders: [
    {
      id: 'o1',
      title: 'Đơn hàng #SC12345 đã được giao thành công!',
      description: 'Cảm ơn bạn đã mua sắm. Hãy đánh giá sản phẩm nhé!',
    ...
    {
      id: 'o2',
      title: 'Đơn hàng #SC12340 đang được vận chuyển.',
      description: 'Dự kiến giao hàng vào ngày mai. Theo dõi hành trình tại đây.',
  ...
  promotions: [
    {
      id: 'p1',
      title: '🔥 Flash Sale giữa tháng sắp bắt đầu!',
      description: 'Hàng ngàn deal sốc giảm đến 70%. Đừng bỏ lỡ vào 12h trưa nay!',
      date: '2025-05-28 08:00',
      read: false,
      icon: Tag,
      type: 'Khuyến mãi',
    },
    {
      id: 'p2',
      title: 'Voucher FREESHIP MAX đã về ví!',
      description: 'Sử dụng ngay cho đơn hàng từ 99K. Số lượng có hạn.',
  ...
  system: [
    {
      id: 's1',
      title: 'Chào mừng bạn đến với ShopeeCharm!',
      description: 'Khám phá ngay hàng ngàn sản phẩm và ưu đãi hấp dẫn.',
    ...
    },
    {
      ...
      description: 'Vui lòng xem chi tiết để đảm bảo quyền lợi của bạn.',
...
const NotificationItem = ({ notification, onMarkRead, onDelete }) => {
  ...
  return (
    ...
    >
      <div
        className={`p-2 rounded-full mt-1 ${!notification.read ? 'bg-primary/20 text-primary' : 'bg-muted text-muted-foreground'}`}
      >
      ...
      <div className='flex-grow'>
        <div className='flex justify-between items-start'>
          <h3
            className={`font-semibold text-sm md:text-md ${!notification.read ? 'text-primary' : ''}`}
          >
            {notification.title}
          </h3>
          {!notification.read && (
      ...
      <div className='flex flex-col space-y-1'>
        {!notification.read && (
          <Button
            variant='ghost'
            size='xs'
            className='text-xs p-1 h-auto text-blue-600 hover:bg-blue-100'
        ...
        <Button
          variant='ghost'
          size='xs'
          className='text-xs p-1 h-auto text-red-500 hover:bg-red-100'
...
const NotificationsPage = () => {
  ...
  const getNotificationsForTab = (tab) => {
    switch (tab) {
      ...
      case 'promotions':
        return notifications.promotions;
      case 'system':
        return notifications.system;
      default:
        return [...notifications.orders, ...notifications.promotions, ...notifications.system].sort(
  ...
  const unreadCount = (type) => {
    if (type === 'all') {
      return (
        notifications.orders.filter((n) => !n.read).length +
      ...
      <Card className='shadow-lg'>
        <Tabs defaultValue='all' onValueChange={setCurrentTab}>
          <TabsList className='grid w-full grid-cols-2 md:grid-cols-4 rounded-none rounded-t-lg border-b h-auto'>
            <TabsTrigger
              value='all'
              className='py-3 text-sm md:text-md data-[state=active]:border-b-2 data-[state=active]:border-primary data-[state=active]:text-primary data-[state=active]:shadow-none rounded-none'
            >
              Tất cả{' '}
              {unreadCount('all') > 0 && (
                <Badge className='ml-1.5 bg-red-500 text-white'>{unreadCount('all')}</Badge>
              )}
            </TabsTrigger>
            <TabsTrigger
              value='orders'
            ...
            >
              ...
              {unreadCount('orders') > 0 && (
                <Badge className='ml-1.5 bg-red-500 text-white'>{unreadCount('orders')}</Badge>
              )}
            </TabsTrigger>
            <TabsTrigger
              value='promotions'
            ...
            >
              Khuyến mãi{' '}
              {unreadCount('promotions') > 0 && (
                <Badge className='ml-1.5 bg-red-500 text-white'>{unreadCount('promotions')}</Badge>
              )}
            </TabsTrigger>
            <TabsTrigger
              value='system'
            ...
            >
              ...
              {unreadCount('system') > 0 && (
                <Badge className='ml-1.5 bg-red-500 text-white'>{unreadCount('system')}</Badge>
              )}
            </TabsTrigger>
          </TabsList>
