...
const UserAccountPage = () => {
  ...
  const [userProfile, setUserProfile] = React.useState({
    ...
    addresses: [
      {
        id: 1,
        fullName: currentUser?.name || 'Người dùng ShopeeCharm',
        phone: '0987654321',
      ...
      },
      {
        ...
        fullName: currentUser?.name || 'Người dùng ShopeeCharm',
        phone: '0912345678',
        address: '456 Đường DEF, Phường UVW, Quận Hai Bà Trưng',
        city: 'Hà Nội',
        isDefault: false,
      },
    ],
    ...
    if (currentUser) {
      ...
      setUserReviews([
        ...
        },
        {
          id: 'REV002',
          productId: '2',
          productName: 'Tai Nghe Bluetooth XYZ',
          rating: 4,
          comment: 'Chất lượng âm thanh tốt, pin hơi nhanh hết.',
          date: '2025-05-18',
        },
      ]);
    }
  ...
  const handleProfileUpdate = (e) => {
    ...
    if (currentUser) {
      ...
      users = users.map((u) =>
        u.id === currentUser.id ? { ...u, name: userProfile.name, phone: userProfile.phone } : u,
      );
  ...
  return (
    <div className='container mx-auto'>
      ...
      <Tabs
        ...
        value={tab || 'profile'}
        onValueChange={(value) => navigate(`/account/${value}`)}
        className='w-full'
      >
        <TabsList className='grid flex-wrap justify-start w-full h-auto grid-cols-3 mb-6 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-9'>
          {tabsConfig.map((t) => (
            <TabsTrigger
              key={t.value}
              value={t.value}
              className='py-2.5 px-3 text-xs md:text-sm data-[state=active]:bg-primary/10 data-[state=active]:text-primary data-[state=active]:shadow-none'
            >
            ...
            <Card className='shadow-lg glassmorphism'>
              <CardHeader className='p-4 border-b md:p-6'>
                <div className='flex flex-col items-start space-y-3 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-4'>
                  <img
                    className='object-cover w-20 h-20 p-1 border-2 rounded-full md:h-24 md:w-24 border-primary'
                    alt={userProfile.avatar_placeholder}
                    src={`https://api.dicebear.com/7.x/initials/svg?seed=${userProfile.name || userProfile.email}&backgroundColor=ff7f50&textColor=ffffff&fontSize=40`}
                  />
                  <div>
            ...
            <Card className='shadow-lg'>
              ...
              <CardContent className='p-4 space-y-4 md:p-6'>
                {userProfile.addresses.map((addr) => (
                  <Card
                    key={addr.id}
                    className={`p-4 ${addr.isDefault ? 'border-primary ring-1 ring-primary' : ''}`}
                  >
                        ...
                        <div className='flex flex-col justify-between sm:flex-row sm:items-center'>
                          ...
                          <div className='mt-2 text-left sm:mt-0 sm:text-right'>
                            <Badge
                              ...
                              className={`mb-2 px-3 py-1 text-xs ${order.status === 'Đã giao' ? 'bg-green-500 text-white' : ''}`}
                            >
                              {order.status}
                            </Badge>
            ...
            <Card className='shadow-lg'>
              ...
              <CardContent className='p-4 md:p-6'>
                {wishlistItems.length > 0 ? (
                  ...
                  <p className='py-8 text-center text-muted-foreground'>
                    Bạn chưa có sản phẩm yêu thích nào. Hãy{' '}
                    ...
                    {userReviews.map((review) => (
                      <Card key={review.id} className='p-3'>
                        ...
                        <p className='mt-1 text-xs text-muted-foreground'>
                          Ngày: {new Date(review.date).toLocaleDateString('vi-VN')}
                        </p>
                      </Card>
                    ))}
                  </div>
              ...
              <CardContent className='p-4 space-y-6 md:p-6'>
                <div className='space-y-3'>
                  <div className='flex items-center justify-between p-3 border rounded-md hover:bg-accent/50 dark:hover:bg-slate-700/30'>
                    <Label htmlFor='notif-updates' className='flex-grow text-sm cursor-pointer'>
                      Cập nhật đơn hàng
                    </Label>
                    <Switch id='notif-updates' defaultChecked />
                  </div>
                  <div className='flex items-center justify-between p-3 border rounded-md hover:bg-accent/50 dark:hover:bg-slate-700/30'>
                    <Label htmlFor='notif-promo' className='flex-grow text-sm cursor-pointer'>
                      Khuyến mãi và ưu đãi
                    </Label>
                    <Switch id='notif-promo' defaultChecked />
                  </div>
                  <div className='flex items-center justify-between p-3 border rounded-md hover:bg-accent/50 dark:hover:bg-slate-700/30'>
                    <Label htmlFor='notif-feed' className='flex-grow text-sm cursor-pointer'>
                      Hoạt động Feed mới
                    </Label>
                    <Switch id='notif-feed' />
                  </div>
                  <div className='flex items-center justify-between p-3 border rounded-md hover:bg-accent/50 dark:hover:bg-slate-700/30'>
                    <Label htmlFor='notif-newsletter' className='flex-grow text-sm cursor-pointer'>
                      Bản tin ShopeeCharm
                    </Label>
            ...
            <Card className='shadow-lg'>
              ...
              <CardContent className='p-4 space-y-4 md:p-6'>
                ...
                <Button variant='outline' className='w-full sm:w-auto sm:ml-2'>
                  Thiết lập mã PIN ShopeeCharmPay
                </Button>
                <Button variant='outline' className='w-full sm:w-auto sm:ml-2'>
              ...
              <CardContent className='p-4 space-y-6 md:p-6'>
                ...
                <div>
                  ...
                  <div className='flex items-center justify-between'>
                    <Label htmlFor='chat-sounds' className='text-sm'>
                      Âm thanh thông báo Chat
                    </Label>
