      ...
      return (
        <div className="container mx-auto">
          ...
          <Tabs defaultValue={tab || "profile"} value={tab || "profile"} onValueChange={(value) => navigate(`/account/${value}`)} className="w-full">
            <TabsList className="grid flex-wrap justify-start w-full h-auto grid-cols-3 mb-6 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-9">
              {tabsConfig.map(t => (
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
              <TabsContent value="profile">
                <Card className="shadow-lg glassmorphism">
                  <CardHeader className="p-4 border-b md:p-6">
                    <div className="flex flex-col items-start space-y-3 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-4">
                       <img  className="object-cover w-20 h-20 p-1 border-2 rounded-full md:h-24 md:w-24 border-primary" alt={userProfile.avatar_placeholder} src={`https://api.dicebear.com/7.x/initials/svg?seed=${userProfile.name || userProfile.email}&backgroundColor=ff7f50&textColor=ffffff&fontSize=40`} />
                      <div>
                        <CardTitle className="text-xl md:text-2xl">{userProfile.name}</CardTitle>
                        <CardDescription>{userProfile.email}</CardDescription>
                        <Button variant="link" size="sm" className="h-auto p-0 mt-1 text-xs text-primary">Thay đổi ảnh đại diện</Button>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent className="p-4 md:p-6">
                    <form onSubmit={handleProfileUpdate} className="space-y-4">
                      ...
                      <Button type="submit" className="gradient-highlands text-primary-foreground">
                        <Edit3 className="w-4 h-4 mr-2" /> Cập nhật hồ sơ
                      </Button>
                    </form>
                  </CardContent>
                </Card>
              ...
              <TabsContent value="addresses">
                <Card className="shadow-lg">
                    <CardHeader className="flex flex-row items-center justify-between p-4 border-b md:p-6">
                        <CardTitle>Sổ địa chỉ</CardTitle>
                        <Button size="sm"><MapPin className="w-4 h-4 mr-2"/>Thêm địa chỉ mới</Button>
                    </CardHeader>
                        ...
                        {userProfile.addresses.map(addr => (
                            <Card key={addr.id} className={`p-4 ${addr.isDefault ? 'border-primary ring-1 ring-primary' : ''}`}>
                                <div className="flex items-start justify-between">
                                    ...
                                    <div className="flex flex-col space-y-1 text-xs sm:flex-row sm:space-y-0 sm:space-x-2">
                                        <Button variant="link" size="sm" className="h-auto p-0 text-primary">Sửa</Button>
                                        {!addr.isDefault && <Button variant="link" size="sm" className="h-auto p-0 text-destructive">Xóa</Button>}
                                    </div>
                                </div>
                ...
                <Card className="shadow-lg">
                  ...
                  <CardContent className="p-4 md:p-6">
                    {orderHistory.length > 0 ? (
                          ...
                          <Card key={order.id} className="p-4 transition-shadow hover:shadow-md">
                            <div className="flex flex-col justify-between sm:flex-row sm:items-center">
                              <div>
                                ...
                                <p className="font-medium text-md">Tổng tiền: {order.totalAmount.toLocaleString()}₫</p>
                              </div>
                      ...
                      <p className="py-8 text-center text-muted-foreground">Bạn chưa có đơn hàng nào.</p>
                    )}
                  </CardContent>
                </Card>
              </TabsContent>
                ...
                <Card className="shadow-lg">
                  ...
                  <CardContent className="p-4 space-y-4 md:p-6">
                    <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
                      <Button className="w-full" variant="outline"><DollarSign className="w-4 h-4 mr-2"/>Nạp tiền</Button>
                      <Button className="w-full" variant="outline">Rút tiền</Button>
                      <Button className="w-full" variant="outline">Chuyển tiền</Button>
                    </div>
                    <div className="pt-4">
                        <h3 className="mb-2 font-semibold">Liên kết ngân hàng</h3>
                        <Button variant="soft" className="w-full border-dashed border-primary text-primary hover:bg-primary/10">
                            <CreditCard className="w-4 h-4 mr-2"/> Liên kết ngay
                        </Button>
                    </div>
                    <h3 className="pt-4 mt-4 font-semibold border-t">Lịch sử giao dịch</h3>
                    <p className="py-4 text-center text-muted-foreground">Chưa có giao dịch nào gần đây.</p>
                  </CardContent>
                </Card>
              </TabsContent>
              ...
              <TabsContent value="wishlist">
                <Card className="shadow-lg">
                  <CardHeader className="p-4 border-b md:p-6"><CardTitle>Danh sách yêu thích ({wishlistItems.length})</CardTitle></CardHeader>
                  <CardContent className="p-4 md:p-6">
                    {wishlistItems.length > 0 ? (
                      ...
                      <p className="py-8 text-center text-muted-foreground">Bạn chưa có sản phẩm yêu thích nào. Hãy <Link to="/" className="text-primary hover:underline">khám phá</Link> thêm nhé!</p>
                    )}
                  </CardContent>
                </Card>
              </TabsContent>
              ...
              <TabsContent value="reviews">
                <Card className="shadow-lg">
                  <CardHeader className="p-4 border-b md:p-6"><CardTitle>Đánh giá của tôi</CardTitle></CardHeader>
                  <CardContent className="p-4 md:p-6">
                        ...
                        <div className="space-y-4">
                            {userReviews.map(review => (
                                <Card key={review.id} className="p-3">
                                    ...
                                    <p className="mt-1 text-xs text-muted-foreground">Ngày: {new Date(review.date).toLocaleDateString('vi-VN')}</p>
                                </Card>
                            ))}
                        </div>
                     ) : (
                        <p className="py-8 text-center text-muted-foreground">Bạn chưa viết đánh giá nào.</p>
                     )}
                  </CardContent>
                </Card>
              </TabsContent>
              ...
              <TabsContent value="notifications">
                <Card className="shadow-lg">
                  <CardHeader className="p-4 border-b md:p-6"><CardTitle>Cài đặt thông báo</CardTitle></CardHeader>
                  <CardContent className="p-4 space-y-6 md:p-6">
                    <div className="space-y-3">
                        <div className="flex items-center justify-between p-3 border rounded-md hover:bg-accent/50 dark:hover:bg-slate-700/30">
                          <Label htmlFor="notif-updates" className="flex-grow text-sm cursor-pointer">Cập nhật đơn hàng</Label>
                          <Switch id="notif-updates" defaultChecked />
                        </div>
                        <div className="flex items-center justify-between p-3 border rounded-md hover:bg-accent/50 dark:hover:bg-slate-700/30">
                          <Label htmlFor="notif-promo" className="flex-grow text-sm cursor-pointer">Khuyến mãi và ưu đãi</Label>
                          <Switch id="notif-promo" defaultChecked />
                        </div>
                        <div className="flex items-center justify-between p-3 border rounded-md hover:bg-accent/50 dark:hover:bg-slate-700/30">
                          <Label htmlFor="notif-feed" className="flex-grow text-sm cursor-pointer">Hoạt động Feed mới</Label>
                          <Switch id="notif-feed" />
                        </div>
                        <div className="flex items-center justify-between p-3 border rounded-md hover:bg-accent/50 dark:hover:bg-slate-700/30">
                          <Label htmlFor="notif-newsletter" className="flex-grow text-sm cursor-pointer">Bản tin ShopeeCharm</Label>
              ...
              <TabsContent value="security">
                <Card className="shadow-lg">
                  <CardHeader className="p-4 border-b md:p-6"><CardTitle>Bảo mật tài khoản</CardTitle></CardHeader>
                  <CardContent className="p-4 space-y-4 md:p-6">
              ...
              <TabsContent value="settings">
                <Card className="shadow-lg">
                  <CardHeader className="p-4 border-b md:p-6"><CardTitle>Cài đặt chung</CardTitle></CardHeader>
                  <CardContent className="p-4 space-y-6 md:p-6">
                     <div>
                      <h3 className="mb-2 font-semibold text-md">Ngôn ngữ & Khu vực</h3>
                      <p className="text-sm text-muted-foreground">Tiếng Việt - Việt Nam (Mặc định)</p>
                      <Button variant="link" size="sm" className="h-auto p-0 mt-1 text-xs text-primary">Thay đổi</Button>
                    </div>
                     <div>
                      <h3 className="mb-2 font-semibold text-md">Cài đặt Chat</h3>
