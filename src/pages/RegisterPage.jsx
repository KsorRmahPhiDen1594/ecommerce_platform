            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Button type="button" variant="ghost" size="icon" className="absolute right-1 top-1/2 -translate-y-1/2 h-8 w-8" onClick={() => setShowConfirmPassword(!showConfirmPassword)} disabled={isLoading}>
                    {showConfirmPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                  </Button>
                </div>
              </div>
              ...
              <Button type="submit" className="w-full gradient-shopee text-primary-foreground text-md py-3" disabled={isLoading || !agreedToTerms}>
                {isLoading ? 'Đang xử lý...' : <><UserPlus className="mr-2 h-5 w-5" /> Đăng Ký</>}
              </Button>
            </form>
          </CardContent>
